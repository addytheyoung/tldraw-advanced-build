...
import { Vec as Vec43 } from "@tldraw/vec";
import {
  Utils as Utils53
} from "@tldraw/core";
...
import { Utils as Utils6, SVGContainer } from "@tldraw/core";

// src/state/shapes/TDShapeUtil.tsx
import { Utils as Utils3, TLShapeUtil } from "@tldraw/core";
import {
...
var stickyFills = {
  light: __spreadProps(__spreadValues({}, Object.fromEntries(Object.entries(colors).map(([k, v]) => [k, Utils.lerpColor(v, canvasLight, 0.45)]))), {
    ["white" /* White */]: "#ffffff",
    ["black" /* Black */]: "#3d3d3d"
  }),
...
var strokes = {
  ...
  dark: __spreadProps(__spreadValues({}, Object.fromEntries(Object.entries(colors).map(([k, v]) => [k, Utils.lerpColor(v, canvasDark, 0.1)]))), {
    ["white" /* White */]: "#cecece",
    ["black" /* Black */]: "#cecece"
  })
};
var fills = {
  light: __spreadProps(__spreadValues({}, Object.fromEntries(Object.entries(colors).map(([k, v]) => [k, Utils.lerpColor(v, canvasLight, 0.82)]))), {
    ["white" /* White */]: "#fefefe"
  }),
  dark: __spreadProps(__spreadValues({}, Object.fromEntries(Object.entries(colors).map(([k, v]) => [k, Utils.lerpColor(v, canvasDark, 0.82)]))), {
...
function transformRectangle(shape, bounds, { initialShape, transformOrigin, scaleX, scaleY }) {
  if (shape.rotation || initialShape.isAspectRatioLocked) {
    const size = Vec.toFixed(Vec.mul(initialShape.size, Math.min(Math.abs(scaleX), Math.abs(scaleY))));
...
var TDShapeUtil = class extends TLShapeUtil {
  constructor() {
    ...
    __publicField(this, "hitTestPoint", (shape, point) => {
      return Utils3.pointInBounds(point, this.getRotatedBounds(shape));
    });
    __publicField(this, "hitTestLineSegment", (shape, A, B) => {
      const box = Utils3.getBoundsFromPoints([A, B]);
      const bounds = this.getBounds(shape);
      return Utils3.boundsContain(bounds, box) || shape.rotation ? intersectLineSegmentPolyline(A, B, Utils3.getRotatedCorners(this.getBounds(shape))).didIntersect : intersectLineSegmentBounds(A, B, this.getBounds(shape)).length > 0;
    });
    ...
    __publicField(this, "getCenter", (shape) => {
      return Utils3.getBoundsCenter(this.getBounds(shape));
    });
    __publicField(this, "getExpandedBounds", (shape) => {
      return Utils3.expandBounds(this.getBounds(shape), this.bindingDistance);
    });
    __publicField(this, "getBindingPoint", (shape, fromShape, point, origin, direction, bindAnywhere) => {
      ...
      if (!Utils3.pointInBounds(point, expandedBounds))
        return;
      ...
      const intersection = intersections.sort((a, b) => Vec3.dist(b, origin) - Vec3.dist(a, origin))[0];
      const middlePoint = Vec3.med(point, intersection);
      let anchor;
      let distance;
      if (bindAnywhere) {
        anchor = Vec3.dist(point, center) < BINDING_DISTANCE / 2 ? center : point;
        distance = 0;
      } else {
        if (Vec3.distanceToLineSegment(point, middlePoint, center) < BINDING_DISTANCE / 2) {
          anchor = center;
        } else {
          anchor = middlePoint;
        }
        if (Utils3.pointInBounds(point, bounds)) {
          distance = this.bindingDistance;
        } else {
          distance = Math.max(this.bindingDistance, Utils3.getBoundsSides(bounds).map((side) => Vec3.distanceToLineSegment(side[1][0], side[1][1], point)).sort((a, b) => a - b)[0]);
        }
      }
      const bindingPoint = Vec3.divV(Vec3.sub(anchor, [expandedBounds.minX, expandedBounds.minY]), [
        expandedBounds.width,
        expandedBounds.height
      ]);
      return {
        point: Vec3.clampV(bindingPoint, 0, 1),
        distance
      };
    });
...
import { Utils as Utils4 } from "@tldraw/core";
import Vec4 from "@tldraw/vec";
import getStroke, { getStrokePoints } from "perfect-freehand";
function getRectangleDrawPoints(id, style, size) {
  const styles2 = getShapeStyle(style);
  const getRandom = Utils4.rng(id);
...
var DashedRectangle = React5.memo(function DashedRectangle2({
  ...
  const paths = strokes2.map(([start, end, length], i) => {
    const { strokeDasharray, strokeDashoffset } = Utils5.getPerfectDashProps(length, strokeWidth * 1.618, style.dash);
...
var RectangleUtil = class extends TDShapeUtil {
  constructor() {
    ...
    __publicField(this, "Component", TDShapeUtil.Component(({
      ...
      }), /* @__PURE__ */ React7.createElement(SVGContainer, {
        id: shape.id + "_svg",
...
import { Utils as Utils9, SVGContainer as SVGContainer2 } from "@tldraw/core";
import {
  intersectBoundsPolygon,
  intersectLineSegmentPolyline as intersectLineSegmentPolyline2,
  intersectRayLineSegment
} from "@tldraw/intersect";
import Vec8 from "@tldraw/vec";
...
__publicField(PolygonUtils, "leftSide", Vec5.isLeft);
...
function getTriangleDrawPoints(id, size, strokeWidth) {
  ...
  const lines = Utils7.rotateArray([
...
function getTrianglePath(id, size, style) {
  ...
  return Utils7.getSvgPathFromStroke(stroke);
}
function getTriangleIndicatorPathTDSnapshot(id, size, style) {
  const { points, options: options2 } = getDrawStrokeInfo2(id, size, style);
  const strokePoints = getStrokePoints2(points, options2);
  return Utils7.getSvgPathFromStroke(strokePoints.map((pt) => pt.point.slice(0, 2)), false);
}
...
var DashedTriangle = React9.memo(function DashedTriangle2({
  ...
  const sides = Utils8.pointsToLineSegments(points, true);
  const paths = sides.map(([start, end], i) => {
    const { strokeDasharray, strokeDashoffset } = Utils8.getPerfectDashProps(Vec7.dist(start, end), strokeWidth * 1.618, style.dash);
...
var TriangleUtil = class extends TDShapeUtil {
  constructor() {
    ...
    __publicField(this, "Component", TDShapeUtil.Component(({
      ...
      return /* @__PURE__ */ React11.createElement(FullWrapper2, __spreadValues({
        ...
        onChange: handleLabelChange,
        onBlur: onShapeBlur
      }), /* @__PURE__ */ React11.createElement(SVGContainer2, {
        id: shape.id + "_svg",
        opacity: isGhost ? GHOSTED_OPACITY : 1
    ...
    __publicField(this, "getExpandedBounds", (shape) => {
      return Utils9.getBoundsFromPoints(getTrianglePoints(shape.size, this.bindingDistance).map((pt) => Vec8.add(pt, shape.point)));
    });
    __publicField(this, "hitTestLineSegment", (shape, A, B) => {
      return intersectLineSegmentPolyline2(A, B, this.getPoints(shape)).didIntersect;
    });
    __publicField(this, "hitTestBounds", (shape, bounds) => {
      return Utils9.boundsContained(this.getBounds(shape), bounds) || intersectBoundsPolygon(bounds, this.getPoints(shape)).length > 0;
    });
    __publicField(this, "getBindingPoint", (shape, fromShape, point, origin, direction, bindAnywhere) => {
      const expandedBounds = this.getExpandedBounds(shape);
      if (!Utils9.pointInBounds(point, expandedBounds))
        return;
      const points = getTrianglePoints(shape.size).map((pt) => Vec8.add(pt, shape.point));
      const expandedPoints = getTrianglePoints(shape.size, this.bindingDistance).map((pt) => Vec8.add(pt, shape.point));
      const closestDistanceToEdge = Utils9.pointsToLineSegments(points, true).map(([a, b]) => Vec8.distanceToLineSegment(a, b, point)).sort((a, b) => a - b)[0];
      if (!(Utils9.pointInPolygon(point, expandedPoints) || closestDistanceToEdge < this.bindingDistance))
        return;
      const intersections = Utils9.pointsToLineSegments(expandedPoints.concat([expandedPoints[0]])).map((segment) => intersectRayLineSegment(origin, direction, segment[0], segment[1])).filter((intersection2) => intersection2.didIntersect).flatMap((intersection2) => intersection2.points);
      if (!intersections.length)
        return;
      const center = Vec8.add(getTriangleCentroid(shape.size), shape.point);
      const intersection = intersections.sort((a, b) => Vec8.dist(b, origin) - Vec8.dist(a, origin))[0];
      const middlePoint = Vec8.med(point, intersection);
      let anchor;
      let distance;
      if (bindAnywhere) {
        anchor = Vec8.dist(point, center) < BINDING_DISTANCE / 2 ? center : point;
        distance = 0;
      } else {
        if (Vec8.distanceToLineSegment(point, middlePoint, center) < BINDING_DISTANCE / 2) {
          anchor = center;
        } else {
          anchor = middlePoint;
        }
        if (Utils9.pointInPolygon(point, points)) {
          distance = this.bindingDistance;
        } else {
          distance = Math.max(this.bindingDistance, closestDistanceToEdge);
        }
      }
      const bindingPoint = Vec8.divV(Vec8.sub(anchor, [expandedBounds.minX, expandedBounds.minY]), [
        expandedBounds.width,
        expandedBounds.height
      ]);
      return {
        point: Vec8.clampV(bindingPoint, 0, 1),
        distance
      };
    });
    __publicField(this, "transform", transformRectangle);
  ...
  getPoints(shape) {
    ...
    return [
      ...
      [x, y + h],
      [x + w, y + h]
    ].map((pt) => Vec8.rotWith(pt, this.getCenter(shape), rotation));
  }
};
...
import { Utils as Utils12, SVGContainer as SVGContainer3 } from "@tldraw/core";
import { Vec as Vec9 } from "@tldraw/vec";
import {
...
import { Utils as Utils10 } from "@tldraw/core";
...
function getEllipsePath(id, radius, style) {
  ...
  const ry = radius[1] + getRandom() * strokeWidth * 2;
  const perimeter = Utils10.perimeterOfEllipse(rx, ry);
  return Utils10.getSvgPathFromStroke(getStrokeOutlinePoints(getEllipseStrokePoints(id, radius, style), {
...
function getEllipseIndicatorPath(id, radius, style) {
  return Utils10.getSvgPathFromStroke(getEllipseStrokePoints(id, radius, style).map((pt) => pt.point.slice(0, 2)), false);
}
...
var DashedEllipse = React13.memo(function DashedEllipse2({
  ...
  const { strokeDasharray, strokeDashoffset } = Utils11.getPerfectDashProps(perimeter < 64 ? perimeter * 2 : perimeter, strokeWidth * 1.618, style.dash, 4);
  ...
  constructor() {
    ...
    __publicField(this, "Component", TDShapeUtil.Component(({
      ...
      return /* @__PURE__ */ React14.createElement(FullWrapper3, __spreadValues({
        ...
        offsetX: (labelPoint[0] - 0.5) * bounds.width,
        offsetY: (labelPoint[1] - 0.5) * bounds.height
      }), /* @__PURE__ */ React14.createElement(SVGContainer3, {
        id: shape.id + "_svg",
        opacity: isGhost ? GHOSTED_OPACITY : 1
    ...
    __publicField(this, "hitTestPoint", (shape, point) => {
      return Utils12.pointInBounds(point, this.getRotatedBounds(shape)) && Utils12.pointInEllipse(point, this.getCenter(shape), shape.radius[0], shape.radius[1], shape.rotation || 0);
    });
    __publicField(this, "hitTestLineSegment", (shape, A, B) => {
      return intersectLineSegmentEllipse(A, B, this.getCenter(shape), shape.radius[0], shape.radius[1], shape.rotation || 0).didIntersect;
    });
    __publicField(this, "getBounds", (shape) => {
      return Utils12.getFromCache(this.boundsCache, shape, () => {
        return Utils12.getRotatedEllipseBounds(shape.point[0], shape.point[1], shape.radius[0], shape.radius[1], 0);
      });
    });
    __publicField(this, "getRotatedBounds", (shape) => {
      return Utils12.getRotatedEllipseBounds(shape.point[0], shape.point[1], shape.radius[0], shape.radius[1], shape.rotation);
    });
    __publicField(this, "hitTestBounds", (shape, bounds) => {
      const shapeBounds = this.getBounds(shape);
      return Utils12.boundsContained(shapeBounds, bounds) || intersectEllipseBounds(this.getCenter(shape), shape.radius[0], shape.radius[1], shape.rotation || 0, bounds).length > 0;
    });
    __publicField(this, "shouldRender", (prev, next) => {
      return next.radius !== prev.radius || next.style !== prev.style || next.label !== prev.label;
    });
    __publicField(this, "getCenter", (shape) => {
      return Vec9.add(shape.point, shape.radius);
    });
    __publicField(this, "getBindingPoint", (shape, fromShape, point, origin, direction, bindAnywhere) => {
      {
        const expandedBounds = this.getExpandedBounds(shape);
        ...
        let bindingPoint;
        let distance;
        if (!Utils12.pointInEllipse(point, center, shape.radius[0] + this.bindingDistance, shape.radius[1] + this.bindingDistance)) {
          return;
        }
        if (bindAnywhere) {
          if (Vec9.dist(point, this.getCenter(shape)) < 12) {
            bindingPoint = [0.5, 0.5];
          } else {
            bindingPoint = Vec9.divV(Vec9.sub(point, [expandedBounds.minX, expandedBounds.minY]), [
              expandedBounds.width,
              expandedBounds.height
            ]);
          }
          ...
          let intersection = intersectRayEllipse(origin, direction, center, shape.radius[0], shape.radius[1], shape.rotation || 0).points.sort((a, b) => Vec9.dist(a, origin) - Vec9.dist(b, origin))[0];
          if (!intersection) {
            intersection = intersectLineSegmentEllipse(point, center, center, shape.radius[0], shape.radius[1], shape.rotation || 0).points.sort((a, b) => Vec9.dist(a, point) - Vec9.dist(b, point))[0];
          }
          if (!intersection) {
            return void 0;
          }
          const anchor = Vec9.med(point, intersection);
          if (Vec9.distanceToLineSegment(point, anchor, this.getCenter(shape)) < 12) {
            bindingPoint = [0.5, 0.5];
          } else {
            bindingPoint = Vec9.divV(Vec9.sub(anchor, [expandedBounds.minX, expandedBounds.minY]), [
              expandedBounds.width,
              expandedBounds.height
            ]);
          }
          if (Utils12.pointInEllipse(point, center, shape.radius[0], shape.radius[1], shape.rotation || 0)) {
            ...
            if (!innerIntersection)
              return void 0;
            distance = Math.max(this.bindingDistance / 2, Vec9.dist(point, innerIntersection));
          }
        }
        return {
          point: bindingPoint,
    ...
    __publicField(this, "transformSingle", (shape, bounds) => {
      return {
        point: Vec9.toFixed([bounds.minX, bounds.minY]),
        radius: Vec9.div([bounds.width, bounds.height], 2)
      };
    });
  }
};
...
import { Utils as Utils16, SVGContainer as SVGContainer4 } from "@tldraw/core";
...
function getBendPoint(handles, bend) {
  ...
  const point = Vec10.toFixed(Math.abs(bendDist) < 10 ? midPoint : Vec10.add(midPoint, Vec10.mul(Vec10.per(u), bendDist)));
  return point;
}
function renderFreehandArrowShaft(id, style, start, end, decorationStart, decorationEnd) {
  const getRandom = Utils13.rng(id);
  const strokeWidth = getShapeStyle(style).strokeWidth;
  const startPoint = decorationStart ? Vec10.nudge(start, end, strokeWidth) : start;
  const endPoint = decorationEnd ? Vec10.nudge(end, start, strokeWidth) : end;
  ...
  return Utils13.getSvgPathFromStroke(stroke);
}
function renderCurvedFreehandArrowShaft(id, style, start, end, decorationStart, decorationEnd, center, radius, length, easing) {
  const getRandom = Utils13.rng(id);
  const strokeWidth = getShapeStyle(style).strokeWidth;
  const startPoint = decorationStart ? Vec10.rotWith(start, center, strokeWidth / length) : start;
  const endPoint = decorationEnd ? Vec10.rotWith(end, center, -(strokeWidth / length)) : end;
  const startAngle = Vec10.angle(center, startPoint);
  const endAngle = Vec10.angle(center, endPoint);
  const points = [];
  const count = 8 + Math.floor(Math.abs(length) / 20 * 1 + getRandom() / 2);
  for (let i = 0; i < count; i++) {
    const t = easing(i / count);
    const angle = Utils13.lerpAngles(startAngle, endAngle, t);
    points.push(Vec10.toFixed(Vec10.nudgeAtAngle(center, angle, radius)));
  }
...
function getCtp(start, bend, end) {
  return Utils13.circleFromThreePoints(start, end, bend);
}
function getCurvedArrowHeadPoints(A, r1, C, r2, sweep) {
  ...
  const left = int ? Vec10.nudge(Vec10.rotWith(int, A, Math.PI / 6), A, r1 * -0.382) : A;
  const right = int ? Vec10.nudge(Vec10.rotWith(int, A, -Math.PI / 6), A, r1 * -0.382) : A;
  return { left, right };
}
function getStraightArrowHeadPoints(A, B, r) {
  ...
  const int = ints[0];
  const left = int ? Vec10.rotWith(int, A, Math.PI / 6) : A;
  const right = int ? Vec10.rotWith(int, A, -Math.PI / 6) : A;
  return { left, right };
}
...
function getArrowPath(style, start, bend, end, decorationStart, decorationEnd) {
  ...
  const arrowDist = Vec10.dist(start, end);
  const arrowHeadLength = Math.min(arrowDist / 3, strokeWidth * 8);
  const path = [];
  const isStraightLine = Vec10.dist(bend, Vec10.toFixed(Vec10.med(start, end))) < 1;
  if (isStraightLine) {
...
function getArcPoints(start, bend, end) {
  if (Vec10.dist2(bend, Vec10.med(start, end)) <= 4)
    return [start, end];
  const points = [];
  const circle = getCtp(start, bend, end);
  const center = [circle[0], circle[1]];
  const radius = circle[2];
  const startAngle = Vec10.angle(center, start);
  const endAngle = Vec10.angle(center, end);
  for (let i = 1 / 20; i < 1; i += 1 / 20) {
    const angle = Utils13.lerpAngles(startAngle, endAngle, i);
    points.push(Vec10.nudgeAtAngle(center, angle, radius));
  }
  return points;
}
...
function getArcLength(C, r, A, B) {
  const sweep = Utils13.getSweep(C, A, B);
  return r * (2 * Math.PI) * (sweep / (2 * Math.PI));
}

// src/state/shapes/ArrowUtil/components/StraightArrow.tsx
import { Utils as Utils14 } from "@tldraw/core";
import Vec11 from "@tldraw/vec";
...
var StraightArrow = React16.memo(function StraightArrow2({
  ...
  const arrowDist = Vec11.dist(start, end);
  if (arrowDist < 2)
    return null;
  const styles2 = getShapeStyle(style, isDarkMode);
  const { strokeWidth } = styles2;
  const sw = 1 + strokeWidth * 1.618;
  const path = isDraw ? renderFreehandArrowShaft(id, style, start, end, decorationStart, decorationEnd) : "M" + Vec11.toFixed(start) + "L" + Vec11.toFixed(end);
  const { strokeDasharray, strokeDashoffset } = Utils14.getPerfectDashProps(arrowDist, strokeWidth * 1.618, style.dash, 2, false);
  const arrowHeadLength = Math.min(arrowDist / 3, strokeWidth * 8);
...
var CurvedArrow = React17.memo(function CurvedArrow2({
  ...
  const arrowDist = Vec12.dist(start, end);
  if (arrowDist < 2)
    return null;
  const styles2 = getShapeStyle(style, isDarkMode);
  const { strokeWidth } = styles2;
  ...
  const radius = circle[2];
  const length = getArcLength(center, radius, start, end);
  const getRandom = Utils15.rng(id);
  ...
  const { strokeDasharray, strokeDashoffset } = Utils15.getPerfectDashProps(Math.abs(length), sw, style.dash, 2, false);
  const arrowHeadLength = Math.min(arrowDist / 3, strokeWidth * 8);
...
var ArrowUtil = class extends TDShapeUtil {
  constructor() {
    ...
    __publicField(this, "Component", TDShapeUtil.Component(({ shape, isEditing, isGhost, meta, events, onShapeChange, onShapeBlur }, ref) => {
      ...
      const isStraightLine = Vec13.dist(bend.point, Vec13.toFixed(Vec13.med(start.point, end.point))) < 1;
      const font = getFontStyle(style);
      const styles2 = getShapeStyle(style, meta.isDarkMode);
      ...
      const dist = React19.useMemo(() => {
        ...
        if (isStraightLine)
          return Vec13.dist(start2.point, end2.point);
      ...
      const offset = React19.useMemo(() => {
        ...
        const offset2 = Vec13.sub(shape.handles.bend.point, Vec13.toFixed([bounds2.width / 2, bounds2.height / 2]));
        return offset2;
      }, [shape, scale]);
      ...
      return /* @__PURE__ */ React19.createElement(FullWrapper4, __spreadValues({
        ...
        onChange: handleLabelChange,
        onBlur: onShapeBlur
      }), /* @__PURE__ */ React19.createElement(SVGContainer4, {
        id: shape.id + "_svg"
    ...
    __publicField(this, "Indicator", TDShapeUtil.Indicator(({ shape, bounds }) => {
      ...
      const offset = React19.useMemo(() => {
        const bounds2 = this.getBounds(shape);
        const offset2 = Vec13.sub(shape.handles.bend.point, [bounds2.width / 2, bounds2.height / 2]);
        return offset2;
      }, [shape, scale]);
    ...
    __publicField(this, "getBounds", (shape) => {
      const bounds = Utils16.getFromCache(this.boundsCache, shape, () => {
        const {
          handles: { start, bend, end }
        } = shape;
        return Utils16.getBoundsFromPoints(getArcPoints(start.point, bend.point, end.point));
      });
      return Utils16.translateBounds(bounds, shape.point);
    });
    __publicField(this, "getRotatedBounds", (shape) => {
      const {
        handles: { start, bend, end }
      ...
      const { minX, minY, maxX, maxY } = Utils16.getBoundsFromPoints(points);
      if (shape.rotation !== 0) {
        points = points.map((pt) => Vec13.rotWith(pt, [(minX + maxX) / 2, (minY + maxY) / 2], shape.rotation || 0));
      }
      return Utils16.translateBounds(Utils16.getBoundsFromPoints(points), shape.point);
    });
    __publicField(this, "getCenter", (shape) => {
      const { start, end } = shape.handles;
      return Vec13.add(shape.point, Vec13.med(start.point, end.point));
    });
    __publicField(this, "shouldRender", (prev, next) => {
    ...
    __publicField(this, "hitTestPoint", (shape, point) => {
      ...
      const pt = Vec13.sub(point, shape.point);
      const points = getArcPoints(start.point, bend.point, end.point);
      for (let i = 1; i < points.length; i++) {
        if (Vec13.distanceToLineSegment(points[i - 1], points[i], pt) < 1) {
          return true;
        }
      }
      return false;
    ...
    __publicField(this, "hitTestLineSegment", (shape, A, B) => {
      ...
      const ptA = Vec13.sub(A, shape.point);
      const ptB = Vec13.sub(B, shape.point);
      const points = getArcPoints(start.point, bend.point, end.point);
      for (let i = 1; i < points.length; i++) {
    ...
    __publicField(this, "hitTestBounds", (shape, bounds) => {
      ...
      const sp = Vec13.add(shape.point, start.point);
      const ep = Vec13.add(shape.point, end.point);
      if (Utils16.pointInBounds(sp, bounds) || Utils16.pointInBounds(ep, bounds)) {
        return true;
      }
      if (Vec13.isEqual(Vec13.med(start.point, end.point), bend.point)) {
        return intersectLineSegmentBounds2(sp, ep, bounds).length > 0;
      } else {
        const [cx, cy, r] = getCtp(start.point, bend.point, end.point);
        const cp = Vec13.add(shape.point, [cx, cy]);
    ...
    __publicField(this, "transform", (shape, bounds, { initialShape, scaleX, scaleY }) => {
      ...
      const dist = Vec13.dist(start.point, end.point);
      const midPoint = Vec13.med(start.point, end.point);
      const bendDist = dist / 2 * initialShape.bend;
      const u = Vec13.uni(Vec13.vec(start.point, end.point));
      const point = Vec13.add(midPoint, Vec13.mul(Vec13.per(u), bendDist));
      nextHandles["bend"] = __spreadProps(__spreadValues({}, bend), {
        point: Vec13.toFixed(Math.abs(bendDist) < 10 ? midPoint : point)
      });
      return {
        point: Vec13.toFixed([bounds.minX, bounds.minY]),
        handles: nextHandles
      };
    });
    ...
    __publicField(this, "onHandleChange", (shape, handles) => {
      let nextHandles = Utils16.deepMerge(shape.handles, handles);
      let nextBend = shape.bend;
      nextHandles = Utils16.deepMerge(nextHandles, {
        start: {
          point: Vec13.toFixed(nextHandles.start.point)
        },
        end: {
          point: Vec13.toFixed(nextHandles.end.point)
        }
      });
      if (Vec13.isEqual(nextHandles.start.point, nextHandles.end.point))
        return;
      if ("bend" in handles) {
        const { start, end, bend } = nextHandles;
        const distance = Vec13.dist(start.point, end.point);
        const midPoint = Vec13.med(start.point, end.point);
        const angle = Vec13.angle(start.point, end.point);
        const u = Vec13.uni(Vec13.vec(start.point, end.point));
        const ap = Vec13.add(midPoint, Vec13.mul(Vec13.per(u), distance));
        const bp = Vec13.sub(midPoint, Vec13.mul(Vec13.per(u), distance));
        const bendPoint = Vec13.nearestPointOnLineSegment(ap, bp, bend.point, true);
        const bendDist = Vec13.dist(midPoint, bendPoint);
        const realBend = bendDist / (distance / 2);
        nextBend = Utils16.clamp(realBend, -0.99, 0.99);
        const angleToBend = Vec13.angle(start.point, bendPoint);
        if (Vec13.isEqual(midPoint, getBendPoint(nextHandles, nextBend))) {
          nextBend = 0;
      ...
      const offset = Vec13.sub([nextBounds.minX, nextBounds.minY], topLeft);
      if (!Vec13.isEqual(offset, [0, 0])) {
        Object.values(nextShape.handles).forEach((handle) => {
          handle.point = Vec13.toFixed(Vec13.sub(handle.point, offset));
        });
        nextShape.point = Vec13.toFixed(Vec13.add(nextShape.point, offset));
      }
      return nextShape;
    });
  }
...
import { Utils as Utils17, SVGContainer as SVGContainer5 } from "@tldraw/core";
var GroupUtil = class extends TDShapeUtil {
  constructor() {
    ...
    __publicField(this, "Component", TDShapeUtil.Component(({ shape, isBinding, isGhost, isHovered, isSelected, events }, ref) => {
      ...
      return /* @__PURE__ */ React20.createElement(SVGContainer5, __spreadValues({
        ref
...
import { Utils as Utils18, HTMLContainer } from "@tldraw/core";
...
var StickyUtil = class extends TDShapeUtil {
  constructor() {
    ...
    __publicField(this, "Component", TDShapeUtil.Component(({ shape, meta, events, isGhost, isBinding, isEditing, onShapeBlur, onShapeChange }, ref) => {
      ...
      return /* @__PURE__ */ React21.createElement(HTMLContainer, __spreadValues({
        ref
    ...
    __publicField(this, "getSvgElement", (shape) => {
      const bounds = this.getBounds(shape);
      const textBounds = Utils18.expandBounds(bounds, -PADDING);
...
import { Utils as Utils19, HTMLContainer as HTMLContainer2 } from "@tldraw/core";
...
var TextUtil = class extends TDShapeUtil {
  constructor() {
    ...
    __publicField(this, "Component", TDShapeUtil.Component(({ shape, isBinding, isGhost, isEditing, onShapeBlur, onShapeChange, meta, events }, ref) => {
      ...
      const handleChange = React22.useCallback((e) => {
        ...
        switch (shape.style.textAlign) {
          ...
          case "middle" /* Middle */: {
            delta = Vec15.div([nextBounds.width - currentBounds.width, 0], 2);
            break;
          }
          case "end" /* End */: {
        ...
        onShapeChange == null ? void 0 : onShapeChange(__spreadProps(__spreadValues({}, shape), {
          id: shape.id,
          point: Vec15.sub(shape.point, delta),
          text: newText
        }));
      ...
      return /* @__PURE__ */ React22.createElement(HTMLContainer2, __spreadValues({
        ref
    ...
    __publicField(this, "getBounds", (shape) => {
      const bounds = Utils19.getFromCache(this.boundsCache, shape, () => {
        var _a;
        if (!melm2) {
      ...
      return Utils19.translateBounds(bounds, shape.point);
    });
    __publicField(this, "shouldRender", (prev, next) => {
    ...
    __publicField(this, "transformSingle", (shape, bounds, { initialShape, scaleX, scaleY }) => {
      ...
      return {
        point: Vec15.toFixed([bounds.minX, bounds.minY]),
        style: __spreadProps(__spreadValues({}, initialShape.style), {
    ...
    __publicField(this, "onDoubleClickBoundsHandle", (shape) => {
      ...
      return {
        ...
        point: Vec15.toFixed(Vec15.add(shape.point, Vec15.sub(center, newCenter)))
      };
    });
    __publicField(this, "getSvgElement", (shape) => {
      const bounds = this.getBounds(shape);
...
import { Utils as Utils21, SVGContainer as SVGContainer6 } from "@tldraw/core";
...
function getFillPath(shape) {
  if (shape.points.length < 2)
    return "";
  return Utils20.getSvgPathFromStroke(getStrokePoints4(shape.points, getFreehandOptions(shape)).map((pt) => pt.point));
}
...
function getDrawStrokePathTDSnapshot(shape) {
  ...
  const path = Utils20.getSvgPathFromStroke(getStrokeOutlinePoints2(strokePoints, options2));
  return path;
}
function getSolidStrokePathTDSnapshot(shape) {
  ...
  if (!Vec16.isEqual(strokePoints[0], last))
    strokePoints.push(last);
  const path = Utils20.getSvgPathFromStroke(strokePoints, false);
  return path;
}
  ...
  constructor() {
    ...
    __publicField(this, "Component", TDShapeUtil.Component(({ shape, meta, isSelected, isGhost, events }, ref) => {
      ...
      if (verySmall) {
        ...
        return /* @__PURE__ */ React23.createElement(SVGContainer6, __spreadValues({
          ref,
          id: shape.id + "_svg"
      ...
      const shouldFill = style.isFilled && points.length > 3 && Vec17.dist(points[0], points[points.length - 1]) < strokeWidth * 2;
    ...
    __publicField(this, "transform", (shape, bounds, { initialShape, scaleX, scaleY }) => {
      const initialShapeBounds = Utils21.getFromCache(this.boundsCache, initialShape, () => Utils21.getBoundsFromPoints(initialShape.points));
      ...
      const newBounds = Utils21.getBoundsFromPoints(shape.points);
      const point = Vec17.sub([bounds.minX, bounds.minY], [newBounds.minX, newBounds.minY]);
      return {
        points,
        point
      };
    ...
    __publicField(this, "getBounds", (shape) => {
      ...
      if (pointsHaveChanged) {
        const bounds = Utils21.getBoundsFromPoints(shape.points);
        this.pointsBoundsCache.set(shape.points, bounds);
        this.shapeBoundsCache.set(shape.id, Utils21.translateBounds(bounds, shape.point));
        this.pointCache[shape.id] = shape.point;
      } else if (pointHasChanged && !pointsHaveChanged) {
        this.pointCache[shape.id] = shape.point;
        this.shapeBoundsCache.set(shape.id, Utils21.translateBounds(this.pointsBoundsCache.get(shape.points), shape.point));
      }
    ...
    __publicField(this, "hitTestPoint", (shape, point) => {
      const ptA = Vec17.sub(point, shape.point);
      return Utils21.pointInPolyline(ptA, shape.points);
    });
    __publicField(this, "hitTestLineSegment", (shape, A, B) => {
      const { points, point } = shape;
      const ptA = Vec17.sub(A, point);
      const ptB = Vec17.sub(B, point);
      const bounds = this.getBounds(shape);
      if (points.length <= 2) {
        return Vec17.distanceToLineSegment(A, B, shape.point) < 4;
      }
    ...
    __publicField(this, "hitTestBounds", (shape, bounds) => {
      if (!shape.rotation) {
        const shapeBounds = this.getBounds(shape);
        return Utils21.boundsContain(bounds, shapeBounds) || (Utils21.boundsContain(shapeBounds, bounds) || intersectBoundsBounds(shapeBounds, bounds).length > 0) && intersectBoundsPolyline(Utils21.translateBounds(bounds, Vec17.neg(shape.point)), shape.points).length > 0;
      }
      const rBounds = this.getRotatedBounds(shape);
      const rotatedBounds = Utils21.getFromCache(this.rotatedCache, shape, () => {
        const c = Utils21.getBoundsCenter(Utils21.getBoundsFromPoints(shape.points));
        return shape.points.map((pt) => Vec17.rotWith(pt, c, shape.rotation || 0));
      });
      return Utils21.boundsContain(bounds, rBounds) || intersectBoundsPolyline(Utils21.translateBounds(bounds, Vec17.neg(shape.point)), rotatedBounds).length > 0;
    });
  }
};
...
import { Utils as Utils22, HTMLContainer as HTMLContainer3 } from "@tldraw/core";
...
var ImageUtil = class extends TDShapeUtil {
  constructor() {
    ...
    __publicField(this, "Component", TDShapeUtil.Component(({ shape, asset = { src: "" }, isBinding, isGhost, meta, events, onShapeChange }, ref) => {
      ...
      return /* @__PURE__ */ React24.createElement(HTMLContainer3, __spreadValues({
        ref
...
import { Utils as Utils23, HTMLContainer as HTMLContainer4 } from "@tldraw/core";
...
var VideoUtil = class extends TDShapeUtil {
  constructor() {
    ...
    __publicField(this, "Component", TDShapeUtil.Component(({ shape, asset = { src: "" }, isBinding, isEditing, isGhost, meta, events, onShapeChange }, ref) => {
      ...
      return /* @__PURE__ */ React25.createElement(HTMLContainer4, __spreadValues({
        ref
...
var _TLDR = class {
  ...
  static getCameraZoom(zoom) {
    return Utils24.clamp(zoom, 0.1, 5);
  }
  ...
  static getSelectedBounds(data) {
    return Utils24.getCommonBounds(_TLDR.getSelectedShapes(data, data.appState.currentPageId).map((shape) => _TLDR.getShapeUtil(shape).getBounds(shape)));
  }
  ...
  static getSelectedBranchSnapshot(data, pageId, fn) {
    const page = _TLDR.getPage(data, pageId);
    const copies = _TLDR.getSelectedIds(data, pageId).flatMap((id) => _TLDR.getDocumentBranch(data, id, pageId).map((id2) => page.shapes[id2])).filter((shape) => !shape.isLocked).map(Utils24.deepClone);
    if (fn !== void 0) {
  ...
  static getSelectedShapeSnapshot(data, pageId, fn) {
    const copies = _TLDR.getSelectedShapes(data, pageId).filter((shape) => !shape.isLocked).map(Utils24.deepClone);
    if (fn !== void 0) {
      return copies.map((shape) => __spreadValues({ id: shape.id }, fn(shape)));
    }
    return copies;
  ...
  static mutateShapes(data, ids, fn, pageId) {
    ...
    const dataWithMutations = Utils24.deepMerge(data, {
      document: {
        pages: {
  ...
  static updateArrowBindings(page, arrowShape) {
    ...
    if (arrowShape.handles.start.bindingId) {
      ...
      const intersectBounds = hasDecoration ? Utils24.expandBounds(bounds, binding.distance) : bounds;
      const { minX, minY, width, height } = expandedBounds;
      const anchorPoint = Vec18.add([minX, minY], Vec18.mulV([width, height], Vec18.rotWith(binding.point, [0.5, 0.5], target.rotation || 0)));
      start = {
        isBound: true,
        hasDecoration,
        binding,
    ...
    for (const ID of ["end", "start"]) {
      ...
      if (A.isBound) {
        if (!A.binding.distance) {
          result[ID].point = Vec18.sub(A.point, arrowShape.point);
        } else {
          const direction = Vec18.uni(Vec18.sub(A.point, B.point));
          switch (A.target.type) {
            case "ellipse" /* Ellipse */: {
              const hits = intersectRayEllipse2(B.point, direction, A.center, A.target.radius[0] + (A.hasDecoration ? A.binding.distance : 0), A.target.radius[1] + (A.hasDecoration ? A.binding.distance : 0), A.target.rotation || 0).points.sort((a, b) => Vec18.dist(a, B.point) - Vec18.dist(b, B.point));
              if (hits[0] !== void 0) {
                result[ID].point = Vec18.toFixed(Vec18.sub(hits[0], arrowShape.point));
              }
              break;
            }
            case "triangle" /* Triangle */: {
              const targetPoint = A.target.point;
              const points = getTrianglePoints(A.target.size, A.hasDecoration ? BINDING_DISTANCE : 0, A.target.rotation).map((pt) => Vec18.add(pt, targetPoint));
              const hits = Utils24.pointsToLineSegments(points, true).map(([p0, p1]) => intersectRayLineSegment2(B.point, direction, p0, p1)).filter((intersection) => intersection.didIntersect).flatMap((intersection) => intersection.points).sort((a, b) => Vec18.dist(a, B.point) - Vec18.dist(b, B.point));
              if (hits[0] !== void 0) {
                result[ID].point = Vec18.toFixed(Vec18.sub(hits[0], arrowShape.point));
              }
              break;
            ...
            default: {
              const hits = intersectRayBounds2(B.point, direction, A.intersectBounds, A.target.rotation).filter((int) => int.didIntersect).map((int) => int.points[0]).sort((a, b) => Vec18.dist(a, B.point) - Vec18.dist(b, B.point));
              if (!hits[0])
                continue;
              let bHit = void 0;
              if (B.isBound) {
                const bHits = intersectRayBounds2(B.point, direction, B.intersectBounds, B.target.rotation).filter((int) => int.didIntersect).map((int) => int.points[0]).sort((a, b) => Vec18.dist(a, B.point) - Vec18.dist(b, B.point));
                bHit = bHits[0];
              }
              if (B.isBound && (hits.length < 2 || bHit && hits[0] && Math.ceil(Vec18.dist(hits[0], bHit)) < BINDING_DISTANCE * 2.5 || Utils24.boundsContain(A.expandedBounds, B.expandedBounds) || Utils24.boundsCollide(A.expandedBounds, B.expandedBounds))) {
                const shortArrowDirection = Vec18.uni(Vec18.sub(B.point, A.point));
                const shortArrowHits = intersectRayBounds2(A.point, shortArrowDirection, A.bounds, A.target.rotation).filter((int) => int.didIntersect).map((int) => int.points[0]);
                if (!shortArrowHits[0])
                  continue;
                result[ID].point = Vec18.toFixed(Vec18.sub(shortArrowHits[0], arrowShape.point));
                result[ID === "start" ? "end" : "start"].point = Vec18.toFixed(Vec18.add(Vec18.sub(shortArrowHits[0], arrowShape.point), Vec18.mul(shortArrowDirection, Math.min(Vec18.dist(shortArrowHits[0], B.point), BINDING_DISTANCE * 2.5 * (Utils24.boundsContain(B.bounds, A.intersectBounds) ? -1 : 1)))));
              } else if (!B.isBound && (hits[0] && Vec18.dist(hits[0], B.point) < BINDING_DISTANCE * 2.5 || Utils24.pointInBounds(B.point, A.intersectBounds))) {
                ...
                return (_d = (_c = _TLDR.getShapeUtil(arrowShape)).onHandleChange) == null ? void 0 : _d.call(_c, arrowShape, {
                  [ID]: __spreadProps(__spreadValues({}, arrowShape.handles[ID]), {
                    point: Vec18.toFixed(Vec18.add(Vec18.sub(B.point, arrowShape.point), Vec18.mul(shortArrowDirection, BINDING_DISTANCE * 2.5)))
                  })
                });
              } else if (hits[0]) {
                result[ID].point = Vec18.toFixed(Vec18.sub(hits[0], arrowShape.point));
  ...
  static getRotatedShapeMutation(shape, center, origin, delta) {
    ...
    const nextRotation = Utils24.clampRadians((shape.rotation || 0) + delta);
    return {
      point: nextPoint,
...
function alignShapes(app, ids, type) {
  ...
  const commonBounds = Utils25.getCommonBounds(boundsForShapes.map(({ bounds }) => bounds));
...
function createPage(app, center, pageId = Utils26.uniqueId()) {
  const { currentPageId } = app;
...
function getDistributions(initialShapes, type) {
  ...
  const commonBounds = Utils27.getCommonBounds(entries.map(({ bounds }) => bounds));
  const results = [];
  switch (type) {
...
function duplicateShapes(app, ids, point) {
  ...
  shapesToDuplicate.forEach((shape) => {
    const duplicatedId = Utils29.uniqueId();
    before.shapes[duplicatedId] = void 0;
    after.shapes[duplicatedId] = __spreadProps(__spreadValues({}, Utils29.deepClone(shape)), {
      id: duplicatedId,
  ...
  shapesToDuplicate.forEach((shape) => {
    if (shape.children) {
      shape.children.forEach((childId) => {
        ...
        before.shapes[duplicatedId] = void 0;
        after.shapes[duplicatedId] = __spreadProps(__spreadValues({}, Utils29.deepClone(child)), {
          id: duplicatedId,
  ...
  Object.values(page.bindings).filter((binding) => dupedShapeIds.has(binding.fromId) || dupedShapeIds.has(binding.toId)).forEach((binding) => {
    if (dupedShapeIds.has(binding.fromId)) {
      if (dupedShapeIds.has(binding.toId)) {
        const duplicatedBindingId = Utils29.uniqueId();
        const duplicatedBinding = __spreadProps(__spreadValues({}, Utils29.deepClone(binding)), {
  ...
  if (point) {
    const commonBounds = Utils29.getCommonBounds(shapesToMove.map((shape) => TLDR.getBounds(shape)));
    const center = Utils29.getBoundsCenter(commonBounds);
...
import { TLBoundsCorner, Utils as Utils30 } from "@tldraw/core";
function flipShapes(app, ids, type) {
  ...
  const commonBounds = Utils30.getCommonBounds(boundsForShapes);
  const { before, after } = TLDR.mutateShapes(app.state, ids, (shape) => {
    const shapeBounds = TLDR.getBounds(shape);
    switch (type) {
      case "horizontal" /* Horizontal */: {
        const newShapeBounds = Utils30.getRelativeTransformedBoundingBox(commonBounds, commonBounds, shapeBounds, true, false);
      ...
      case "vertical" /* Vertical */: {
        const newShapeBounds = Utils30.getRelativeTransformedBoundingBox(commonBounds, commonBounds, shapeBounds, false, true);
        return TLDR.getShapeUtil(shape).transform(shape, newShapeBounds, {
          type: TLBoundsCorner.TopLeft,
          scaleX: 1,
          scaleY: -1,
...
function groupShapes(app, ids, groupId, pageId) {
  ...
  const groupBounds = Utils31.getCommonBounds(shapesToGroup.map((shape) => TLDR.getBounds(shape)));
...
function moveShapesToPage(app, ids, viewportBounds, fromPageId, toPageId) {
  ...
  const bounds = Utils32.getCommonBounds(movingShapes.map((shape) => TLDR.getBounds(shape)));
...
function rotateShapes(app, ids, delta = -PI23 / 4) {
  ...
  const origin = Utils33.getBoundsCenter(Utils33.getCommonBounds(shapesToRotate.map((shape) => TLDR.getBounds(shape))));
...
import { TLBoundsCorner as TLBoundsCorner2, Utils as Utils34 } from "@tldraw/core";
function stretchShapes(app, ids, type) {
  const { currentPageId, selectedIds } = app;
  const initialShapes = ids.map((id) => app.getShape(id));
  const boundsForShapes = initialShapes.map((shape) => TLDR.getBounds(shape));
  const commonBounds = Utils34.getCommonBounds(boundsForShapes);
...
function styleShapes(app, ids, changes) {
  ...
  shapeIdsToMutate.map((id) => app.getShape(id)).filter((shape) => !shape.isLocked).forEach((shape) => {
    ...
    if (shape.type === "text" /* Text */) {
      ...
      afterShapes[shape.id].point = Vec23.toFixed(Vec23.add(shape.point, Vec23.sub(app.getShapeUtil(shape).getCenter(shape), app.getShapeUtil(shape).getCenter(__spreadProps(__spreadValues({}, shape), {
...
var ArrowSession = class extends BaseSession {
  constructor(app, shapeId, handleId, isCreate = false) {
    ...
    __publicField(this, "newStartBindingId", Utils35.uniqueId());
    __publicField(this, "draggedBindingId", Utils35.uniqueId());
    ...
    __publicField(this, "update", () => {
      ...
      if (!handles[handleId].canBind)
        return;
      let delta = Vec25.sub(currentPoint, Vec25.add(handles[handleId].point, initialShape.point));
      if (shiftKey) {
        const A = altKey ? Vec25.med(handles.start.point, handles.end.point) : handles[handleId === "start" ? "end" : "start"].point;
        const B = handles[handleId].point;
        const C = Vec25.add(B, delta);
        const angle = Vec25.angle(A, C);
        const adjusted = Vec25.rotWith(C, A, Utils35.snapAngleToSegments(angle, 24) - angle);
        delta = Vec25.add(delta, Vec25.sub(adjusted, C));
      }
      const nextPoint = Vec25.add(handles[handleId].point, delta);
      const handleChanges = {
        [handleId]: __spreadProps(__spreadValues({}, handles[handleId]), {
          point: showGrid ? Vec25.snap(nextPoint, currentGrid) : Vec25.toFixed(nextPoint),
          bindingId: void 0
        })
      };
      if (altKey) {
        const oppositeHandleId = handleId === "start" ? "end" : "start";
        const nextPoint2 = Vec25.sub(handles[oppositeHandleId].point, delta);
        handleChanges[oppositeHandleId] = __spreadProps(__spreadValues({}, handles[oppositeHandleId]), {
          point: showGrid ? Vec25.snap(nextPoint2, currentGrid) : Vec25.toFixed(nextPoint2),
          bindingId: void 0
        });
      }
      ...
      if (!handleChange)
        return;
      const next = {
        shape: Utils35.deepMerge(shape, handleChange),
        bindings: {}
      };
      let draggedBinding;
      ...
      if (this.startBindingShapeId) {
        ...
        const rayPoint = Vec25.add(startHandle.point, next.shape.point);
        if (Vec25.isEqual(rayPoint, center))
          rayPoint[1]++;
        const rayOrigin = center;
        const isInsideShape = startTargetUtils.hitTestPoint(startTarget, currentPoint);
        const rayDirection = Vec25.uni(Vec25.sub(rayPoint, rayOrigin));
        const hasStartBinding = this.app.getBinding(this.newStartBindingId) !== void 0;
        if (!metaKey && !startTargetUtils.hitTestPoint(startTarget, Vec25.add(next.shape.point, endHandle.point))) {
        ...
        if (nextStartBinding && !hasStartBinding) {
          ...
          next.shape = Utils35.deepMerge(next.shape, {
            handles: {
              start: {
          ...
          next.shape = Utils35.deepMerge(initialShape, {
            handles: {
              start: {
                bindingId: void 0
              }
      ...
      if (!metaKey) {
        const rayOrigin = Vec25.add(oppositeHandle.point, next.shape.point);
        const rayPoint = Vec25.add(draggingHandle.point, next.shape.point);
        const rayDirection = Vec25.uni(Vec25.sub(rayPoint, rayOrigin));
        const startPoint = Vec25.add(next.shape.point, next.shape.handles.start.point);
        const endPoint = Vec25.add(next.shape.point, next.shape.handles.end.point);
    ...
    __publicField(this, "cancel", () => {
      ...
      const isDeleting = this.isCreate || Vec25.dist(currentShape.handles.start.point, currentShape.handles.end.point) < 4;
      const afterBindings = {};
    ...
    __publicField(this, "complete", () => {
      ...
      const length = Vec25.dist(currentShape.handles.start.point, currentShape.handles.end.point);
    ...
    __publicField(this, "findBindingPoint", (shape, target, handleId, bindingId, point, origin, direction, bindAnywhere) => {
      ...
      return {
        ...
        toId: target.id,
        handleId,
        point: Vec25.toFixed(bindingPoint.point),
    ...
    if (this.isCreate) {
      this.startBindingShapeId = (_c = this.bindableShapeIds.map((id) => page.shapes[id]).filter((shape) => Utils35.pointInBounds(originPoint, TLDR.getShapeUtil(shape).getBounds(shape))).sort((a, b) => {
...
var BrushSession = class extends BaseSession {
  constructor(app) {
    ...
    __publicField(this, "update", () => {
      ...
      const brush = Utils36.getBoundsFromPoints([originPoint, currentPoint]);
      ...
      shapesToTest.forEach(({ id, selectId }) => {
        ...
        if (!hits.has(selectId)) {
          ...
          if (selectByContain ? Utils36.boundsContain(brush, util.getBounds(shape)) : util.hitTestBounds(shape, brush)) {
            hits.add(selectId);
...
import { Utils as Utils37 } from "@tldraw/core";
...
var DrawSession = class extends BaseSession {
  constructor(app, id) {
    ...
    __publicField(this, "start", () => {
      ...
      const delta = Vec26.sub(topLeft, currentPoint);
      this.topLeft = topLeft;
      this.shiftedPoints = this.points.map((pt) => Vec26.toFixed(Vec26.sub(pt, delta)).concat(pt[2]));
      return {
        document: {
          pages: {
            [this.app.currentPageId]: {
    ...
    __publicField(this, "update", () => {
      ...
      if (!this.lockedDirection && this.points.length > 1) {
        const bounds = Utils37.getBoundsFromPoints(this.points);
              ...
              [pageId]: {
                shapes: {
                  [shapeId]: __spreadProps(__spreadValues({}, shape), {
                    point: Vec26.toFixed(shape.point),
                    points: shape.points.map((pt) => Vec26.toFixed(pt)),
                    isComplete: true
                  })
                }
              }
    ...
    __publicField(this, "addPoint", (currentPoint) => {
      const { originPoint } = this.app;
      const newAdjustedPoint = Vec26.toFixed(Vec26.sub(currentPoint, originPoint)).concat(currentPoint[2]);
      if (Vec26.isEqual(this.lastAdjustedPoint, newAdjustedPoint))
        return;
      this.points.push(newAdjustedPoint);
      ...
      const delta = Vec26.sub(topLeft, originPoint);
      let points;
      if (prevTopLeft[0] !== topLeft[0] || prevTopLeft[1] !== topLeft[1]) {
        this.topLeft = topLeft;
        points = this.points.map((pt) => Vec26.toFixed(Vec26.sub(pt, delta)).concat(pt[2]));
      } else {
        points = [...this.shiftedPoints, Vec26.sub(newAdjustedPoint, delta).concat(newAdjustedPoint[2])];
      }
    ...
    const delta = Vec26.sub(originPoint, this.topLeft);
    const initialPoints = this.initialShape.points.map((pt) => Vec26.sub(pt, delta).concat(pt[2]));
    ...
    if (this.isExtending) {
      ...
      const len = Math.ceil(Vec26.dist(prevPoint, currentPoint) / 16);
      for (let i = 0; i < len; i++) {
        const t = i / (len - 1);
        newPoints.push(Vec26.lrp(prevPoint, currentPoint, t).concat(prevPoint[2]));
      }
    } else {
      newPoints.push(currentPoint);
    }
    this.points = [...initialPoints, ...newPoints];
    this.shiftedPoints = this.points.map((pt) => Vec26.add(pt, delta).concat(pt[2]));
...
var HandleSession = class extends BaseSession {
  constructor(app, shapeId, handleId, commandId = "move_handle") {
    ...
    __publicField(this, "update", () => {
      ...
      const handleId = this.handleId;
      const delta = Vec27.sub(currentPoint, handles[handleId].point);
      const handleChanges = {
        [handleId]: __spreadProps(__spreadValues({}, handles[handleId]), {
          point: Vec27.sub(Vec27.add(handles[handleId].point, delta), shape.point)
        })
      };
...
var RotateSession = class extends BaseSession {
  constructor(app) {
    ...
    __publicField(this, "update", () => {
      ...
      const shapes = {};
      let directionDelta = Vec28.angle(commonBoundsCenter, currentPoint) - this.initialAngle;
      if (shiftKey) {
        directionDelta = Utils38.snapAngleToSegments(directionDelta, 24);
      }
      initialShapes.forEach(({ center, shape }) => {
        const { rotation = 0 } = shape;
        let shapeDelta = 0;
        if (shiftKey) {
          const snappedRotation = Utils38.snapAngleToSegments(rotation, 24);
    ...
    if (app.rotationInfo.selectedIds === pageState.selectedIds) {
      ...
      this.commonBoundsCenter = Utils38.getBoundsCenter(Utils38.getCommonBounds(initialShapes.map(TLDR.getBounds)));
    ...
    this.initialAngle = Vec28.angle(this.commonBoundsCenter, originPoint);
  }
};

// src/state/sessions/TransformSession/TransformSession.ts
import { TLBoundsCorner as TLBoundsCorner3, Utils as Utils39 } from "@tldraw/core";
...
var TransformSession = class extends BaseSession {
  constructor(app, transformType = TLBoundsCorner3.BottomRight, isCreate = false) {
    ...
    __publicField(this, "start", () => {
      this.snapInfo = {
        state: "ready",
        bounds: this.app.shapes.filter((shape) => !this.initialShapeIds.includes(shape.id)).map((shape) => Utils39.getBoundsWithCenter(TLDR.getRotatedBounds(shape)))
      };
      return void 0;
    });
    __publicField(this, "update", () => {
      ...
      const shapes = {};
      const delta = altKey ? Vec29.mul(Vec29.sub(currentPoint, originPoint), 2) : Vec29.sub(currentPoint, originPoint);
      let newBounds = Utils39.getTransformedBoundingBox(initialCommonBounds, transformType, delta, 0, shiftKey || isAllAspectRatioLocked);
      if (altKey) {
        newBounds = __spreadValues(__spreadValues({}, newBounds), Utils39.centerBounds(newBounds, Utils39.getBoundsCenter(initialCommonBounds)));
      }
      if (showGrid) {
        newBounds = __spreadValues(__spreadValues({}, newBounds), Utils39.snapBoundsToGrid(newBounds, currentGrid));
      }
      const speed = Vec29.dist(currentPoint, previousPoint);
      ...
      if ((isSnapping && !metaKey || !isSnapping && metaKey) && this.speed * camera.zoom < SLOW_SPEED && this.snapInfo.state === "ready") {
        const snapResult = Utils39.getSnapPoints(Utils39.getBoundsWithCenter(newBounds), this.snapInfo.bounds.filter((bounds) => Utils39.boundsContain(viewport, bounds) || Utils39.boundsCollide(viewport, bounds)), SNAP_DISTANCE / camera.zoom);
        if (snapResult) {
          snapLines = snapResult.snapLines;
          newBounds = Utils39.getTransformedBoundingBox(initialCommonBounds, transformType, Vec29.sub(delta, snapResult.offset), 0, shiftKey || isAllAspectRatioLocked);
        }
      }
      ...
      shapeBounds.forEach(({ initialShape, initialShapeBounds, transformOrigin }) => {
        let newShapeBounds = Utils39.getRelativeTransformedBoundingBox(newBounds, initialCommonBounds, initialShapeBounds, this.scaleX < 0, this.scaleY < 0);
        if (showGrid) {
          newShapeBounds = Utils39.snapBoundsToGrid(newShapeBounds, currentGrid);
        }
    ...
    __publicField(this, "complete", () => {
      ...
      if (!hasUnlockedShapes)
        return;
      if (this.isCreate && Vec29.dist(this.app.originPoint, this.app.currentPoint) < 2) {
        return this.cancel();
      }
      const beforeShapes = {};
      const afterShapes = {};
    ...
    this.initialCommonBounds = Utils39.getCommonBounds(boundsArr);
    const initialInnerBounds = Utils39.getBoundsFromPoints(boundsArr.map(Utils39.getBoundsCenter));
    this.shapeBounds = this.initialShapes.map((shape) => {
      const initialShapeBounds = shapesBounds[shape.id];
      const ic = Utils39.getBoundsCenter(initialShapeBounds);
...
var TransformSingleSession = class extends BaseSession {
  constructor(app, id, transformType, isCreate = false) {
    ...
    __publicField(this, "start", () => {
      this.snapInfo = {
        state: "ready",
        bounds: this.app.shapes.filter((shape) => shape.id !== this.initialShape.id).map((shape) => Utils40.getBoundsWithCenter(TLDR.getRotatedBounds(shape)))
      };
      return void 0;
    });
    __publicField(this, "update", () => {
      ...
      if (initialShape.isLocked)
        return void 0;
      const shapes = {};
      const delta = altKey ? Vec30.mul(Vec30.sub(currentPoint, originPoint), 2) : Vec30.sub(currentPoint, originPoint);
      const shape = this.app.getShape(initialShape.id);
      const utils = TLDR.getShapeUtil(shape);
      let newBounds = Utils40.getTransformedBoundingBox(initialShapeBounds, transformType, delta, shape.rotation, shiftKey || shape.isAspectRatioLocked || utils.isAspectRatioLocked);
      if (altKey) {
        newBounds = __spreadValues(__spreadValues({}, newBounds), Utils40.centerBounds(newBounds, Utils40.getBoundsCenter(initialShapeBounds)));
      }
      if (showGrid) {
        newBounds = __spreadValues(__spreadValues({}, newBounds), Utils40.snapBoundsToGrid(newBounds, currentGrid));
      }
      const speed = Vec30.dist(currentPoint, previousPoint);
      const speedChange = speed - this.speed;
      this.speed = this.speed + speedChange * (speedChange > 1 ? 0.5 : 0.15);
      let snapLines = [];
      if ((isSnapping && !metaKey || !isSnapping && metaKey) && !initialShape.rotation && this.speed * camera.zoom < SLOW_SPEED && this.snapInfo.state === "ready") {
        const snapResult = Utils40.getSnapPoints(Utils40.getBoundsWithCenter(newBounds), this.snapInfo.bounds.filter((bounds) => Utils40.boundsContain(viewport, bounds) || Utils40.boundsCollide(viewport, bounds)), SNAP_DISTANCE / camera.zoom);
        if (snapResult) {
          snapLines = snapResult.snapLines;
          newBounds = Utils40.getTransformedBoundingBox(initialShapeBounds, transformType, Vec30.sub(delta, snapResult.offset), shape.rotation, shiftKey || shape.isAspectRatioLocked || utils.isAspectRatioLocked);
        }
      }
      ...
      if (showGrid && afterShape && afterShape.point) {
        afterShape.point = Vec30.snap(afterShape.point, currentGrid);
      }
      return {
        appState: {
          snapLines
    ...
    __publicField(this, "complete", () => {
      ...
      if (initialShape.isLocked)
        return;
      if (this.isCreate && Vec30.dist(this.app.originPoint, this.app.currentPoint) < 2) {
        return this.cancel();
      }
      const beforeShapes = {};
      const afterShapes = {};
...
var TranslateSession = class extends BaseSession {
  constructor(app, isCreate = false, link = false) {
    ...
    __publicField(this, "start", () => {
      ...
      const otherBounds = [];
      Object.values(page.shapes).forEach((shape) => {
        const bounds = Utils41.getBoundsWithCenter(TLDR.getRotatedBounds(shape));
    ...
    __publicField(this, "update", () => {
      ...
      const nextShapes = {};
      const nextPageState = {};
      let delta = Vec31.sub(currentPoint, originPoint);
      ...
      const speed = Vec31.dist(currentPoint, previousPoint);
      const change = speed - this.speed;
      this.speed = this.speed + change * (change > 1 ? 0.5 : 0.15);
      this.snapLines = [];
      if ((isSnapping && !metaKey || !isSnapping && metaKey) && this.speed * camera.zoom < SLOW_SPEED && this.snapInfo.state === "ready") {
        const snapResult = Utils41.getSnapPoints(Utils41.getBoundsWithCenter(showGrid ? Utils41.snapBoundsToGrid(Utils41.translateBounds(initialCommonBounds, delta), currentGrid) : Utils41.translateBounds(initialCommonBounds, delta)), (this.isCloning ? this.snapInfo.bounds : this.snapInfo.others).filter((bounds) => {
          return Utils41.boundsContain(viewport, bounds) || Utils41.boundsCollide(viewport, bounds);
        }), SNAP_DISTANCE / camera.zoom);
        if (snapResult) {
          this.snapLines = snapResult.snapLines;
          delta = Vec31.sub(delta, snapResult.offset);
        }
      }
      this.prev = delta;
      if (this.isCloning) {
        if (didChangeCloning) {
          ...
          clones.forEach((clone) => {
            nextShapes[clone.id] = __spreadProps(__spreadValues({}, clone), {
              point: showGrid ? Vec31.snap(Vec31.toFixed(Vec31.add(clone.point, delta)), currentGrid) : Vec31.toFixed(Vec31.add(clone.point, delta))
            });
          });
        } else {
          if (this.cloneInfo.state === "empty")
        ...
        if (didChangeCloning) {
          ...
          initialShapes.forEach((shape) => {
            nextShapes[shape.id] = {
              point: showGrid ? Vec31.snap(Vec31.toFixed(Vec31.add(shape.point, delta)), currentGrid) : Vec31.toFixed(Vec31.add(shape.point, delta))
            };
          });
          for (const binding of clonedBindings) {
    ...
    __publicField(this, "createCloneInfo", () => {
      ...
      const clones = [];
      initialShapes.forEach((shape) => {
        const newId = Utils41.uniqueId();
        initialParentChildren[newId] = initialParentChildren[shape.id];
        cloneMap[shape.id] = newId;
        const clone = __spreadProps(__spreadValues({}, Utils41.deepClone(shape)), {
          id: newId,
          parentId: shape.parentId,
      ...
      Object.values(page.bindings).filter((binding) => clonedShapeIds.has(binding.fromId) || clonedShapeIds.has(binding.toId)).forEach((binding) => {
        if (clonedShapeIds.has(binding.fromId)) {
          if (clonedShapeIds.has(binding.toId)) {
            const cloneId = Utils41.uniqueId();
            const cloneBinding = __spreadProps(__spreadValues({}, Utils41.deepClone(binding)), {
              id: cloneId,
    ...
    this.initialCommonBounds = Utils41.getCommonBounds(this.initialShapes.map(TLDR.getRotatedBounds));
...
var EraseSession = class extends BaseSession {
  constructor(app) {
    ...
    __publicField(this, "update", () => {
      ...
      if (shiftKey) {
        if (!this.isLocked && Vec32.dist(originPoint, currentPoint) > 4) {
          if (!this.lockedDirection) {
            const delta = Vec32.sub(currentPoint, originPoint);
      ...
      const newPoint = Vec32.toFixed(Vec32.add(originPoint, Vec32.sub(currentPoint, originPoint)));
...
var GridSession = class extends BaseSession {
  constructor(app, id) {
    ...
    __publicField(this, "update", () => {
      ...
      const nextShapes = {};
      const nextPageState = {};
      const center = Utils42.getBoundsCenter(this.bounds);
      const offset = Vec33.sub(currentPoint, center);
      if (shiftKey) {
      ...
      for (let x = minX; x < maxX; x++) {
        for (let y = minY; y < maxY; y++) {
          ...
          if (x === 0 && y === 0)
            continue;
          const clone = this.getClone(Vec33.add(this.shape.point, [x * gapX, y * gapY]), isCopying);
...
var SelectTool = class extends BaseTool {
  constructor() {
    ...
    __publicField(this, "clonePaint", (point) => {
      ...
      const bounds = Utils44.expandBounds(Utils44.getCommonBounds(shapes.map(TLDR.getBounds)), 16);
      const center = Utils44.getBoundsCenter(bounds);
      ...
      const centeredBounds = Utils44.centerBounds(bounds, gridPoint);
    ...
    __publicField(this, "getShapeClone", (id, side) => {
      ...
      if (utils.canClone) {
        ...
        if (shape.rotation !== 0) {
          const newCenter = Vec34.add(point, [bounds.width / 2, bounds.height / 2]);
          const rotatedCenter = Vec34.rotWith(newCenter, center, shape.rotation || 0);
          point = Vec34.sub(rotatedCenter, [bounds.width / 2, bounds.height / 2]);
        }
        const id2 = Utils44.uniqueId();
    ...
    __publicField(this, "onPointerMove", (info, e) => {
      ...
      switch (this.status) {
        case "pointingBoundsHandle" /* PointingBoundsHandle */: {
          ...
          if (Vec34.dist(originPoint, currentPoint) > DEAD_ZONE) {
    ...
    __publicField(this, "onPointerMove", (info) => {
      switch (this.status) {
        case "pointing" /* Pointing */: {
          if (Vec35.dist(info.origin, info.point) > DEAD_ZONE) {
  ...
  constructor() {
    ...
    __publicField(this, "onPointerDown", () => {
      ...
      if (this.status === "idle" /* Idle */) {
        ...
        this.app.createTextShapeAtPoint(showGrid ? Vec36.snap(currentPoint, currentGrid) : currentPoint);
...
import { Utils as Utils46, TLBoundsCorner as TLBoundsCorner6 } from "@tldraw/core";
...
var EllipseTool = class extends BaseTool {
  constructor() {
    ...
    __publicField(this, "onPointerDown", () => {
      ...
      this.app.patchCreate([newShape]);
      this.app.startSession("transformSingle" /* TransformSingle */, newShape.id, TLBoundsCorner6.BottomRight, true);
      this.setStatus("creating" /* Creating */);
    });
  }
};
...
import { Utils as Utils47, TLBoundsCorner as TLBoundsCorner7 } from "@tldraw/core";
...
var RectangleTool = class extends BaseTool {
  constructor() {
    ...
    __publicField(this, "onPointerDown", () => {
      ...
      this.app.patchCreate([newShape]);
      this.app.startSession("transformSingle" /* TransformSingle */, newShape.id, TLBoundsCorner7.BottomRight, true);
      this.setStatus("creating" /* Creating */);
    });
  }
};
...
import { Utils as Utils48, TLBoundsCorner as TLBoundsCorner8 } from "@tldraw/core";
...
var TriangleTool = class extends BaseTool {
  constructor() {
    ...
    __publicField(this, "onPointerDown", () => {
      ...
      this.app.patchCreate([newShape]);
      this.app.startSession("transformSingle" /* TransformSingle */, newShape.id, TLBoundsCorner8.BottomRight, true);
      this.setStatus("creating" /* Creating */);
    });
  }
};
...
import { Utils as Utils49 } from "@tldraw/core";
  ...
  constructor() {
    ...
    __publicField(this, "onPointerDown", () => {
      ...
      const newShape = Arrow.create({
        ...
        parentId: currentPageId,
        childIndex,
        point: showGrid ? Vec40.snap(currentPoint, currentGrid) : currentPoint,
        decorations: {
          start: void 0,
          end: void 0
        },
...
import { Utils as Utils51 } from "@tldraw/core";
    ...
    __publicField(this, "onPointerDown", () => {
      ...
      if (this.status === "idle" /* Idle */) {
        ...
        const newShape = Sticky.create({
          ...
          parentId: currentPageId,
          childIndex,
          point: showGrid ? Vec42.snap(currentPoint, currentGrid) : currentPoint,
          style: __spreadValues({}, currentStyle)
        });
        const bounds = Sticky.getBounds(newShape);
        newShape.point = Vec42.sub(newShape.point, [bounds.width / 2, bounds.height / 2]);
...
import { Utils as Utils52 } from "@tldraw/core";
var StateManager = class {
  constructor(initialState, id, version, update) {
    ...
    __publicField(this, "applyPatch", (patch, id) => {
      ...
      const next = Utils52.deepMerge(this._state, patch);
...
var _TldrawApp = class extends StateManager {
  constructor(id, callbacks = {}) {
    ...
    __publicField(this, "viewport", Utils53.getBoundsFromPoints([
      [0, 0],
      [100, 100]
    ]));
    __publicField(this, "rendererBounds", Utils53.getBoundsFromPoints([
      [0, 0],
      [100, 100]
    ]));
        ...
        Object.entries(next.document.pages).forEach(([pageId, page]) => {
          ...
          if (!prevPage || page.shapes !== prevPage.shapes || page.bindings !== prevPage.bindings) {
            ...
            groupsToUpdate.forEach((group) => {
              ...
              const commonBounds = Utils53.getCommonBounds(children.map((id) => page.shapes[id]).filter(Boolean).map((shape) => TLDR.getRotatedBounds(shape)));
    ...
    __publicField(this, "replacePageContent", (shapes, bindings, assets, pageId = this.currentPageId) => {
      ...
      this.useStore.setState((current) => {
        ...
        Object.values(nextShapes).forEach((shape) => {
          ...
          const commonBounds = Utils53.getCommonBounds(children.map((id) => page2.shapes[id]).filter(Boolean).map((shape2) => TLDR.getRotatedBounds(shape2)));
    ...
    __publicField(this, "updateViewport", (point, zoom) => {
      ...
      const [minX, minY] = Vec43.sub(Vec43.div([0, 0], zoom), point);
      const [maxX, maxY] = Vec43.sub(Vec43.div([width, height], zoom), point);
      this.viewport = {
        minX,
        minY,
        maxX,
    ...
    __publicField(this, "getPagePoint", (point, pageId = this.currentPageId) => {
      ...
      return Vec43.sub(Vec43.div(point, camera.zoom), camera.point);
    });
    ...
    __publicField(this, "copy", (ids = this.selectedIds) => {
      ...
      const copyingShapes = copyingShapeIds.map((id) => Utils53.deepClone(this.getShape(id, this.currentPageId)));
    ...
    __publicField(this, "paste", (point) => {
      ...
      const pasteInCurrentPage = (shapes, bindings, assets) => {
        ...
        shapes.forEach((shape) => idsMap[shape.id] = Utils53.uniqueId());
        bindings.forEach((binding) => idsMap[binding.id] = Utils53.uniqueId());
        ...
        const commonBounds = Utils53.getCommonBounds(shapesToPaste.map(TLDR.getBounds));
        let center = Vec43.toFixed(this.getPagePoint(point || this.centerPoint));
        if (Vec43.dist(center, this.pasteInfo.center) < 2 || Vec43.dist(center, Vec43.toFixed(Utils53.getBoundsCenter(commonBounds))) < 2) {
          center = Vec43.add(center, this.pasteInfo.offset);
          this.pasteInfo.offset = Vec43.add(this.pasteInfo.offset, [GRID_SIZE, GRID_SIZE]);
        } else {
          this.pasteInfo.center = center;
          this.pasteInfo.offset = [0, 0];
        }
        const centeredBounds = Utils53.centerBounds(commonBounds, center);
        const delta = Vec43.sub(Utils53.getBoundsCenter(centeredBounds), Utils53.getBoundsCenter(commonBounds));
        this.create(shapesToPaste.map((shape) => TLDR.getShapeUtil(shape.type).create(__spreadProps(__spreadValues({}, shape), {
          point: Vec43.toFixed(Vec43.add(shape.point, delta)),
      ...
      navigator.clipboard.readText().then((result) => {
        ...
        if (data.type === "tldr/clipboard") {
          ...
          const shapeId = Utils53.uniqueId();
          this.createShapes({
            id: shapeId,
            type: "text" /* Text */,
    ...
    __publicField(this, "copySvg", (ids = this.selectedIds, pageId = this.currentPageId) => {
      ...
      const commonBounds = Utils53.getCommonBounds(shapes.map(TLDR.getRotatedBounds));
    ...
    __publicField(this, "pan", (delta) => {
      ...
      return this.setCamera(Vec43.toFixed(Vec43.sub(camera.point, delta)), camera.zoom, `panned`);
    });
    __publicField(this, "pinchZoom", (point, delta, zoom) => {
      const { camera } = this.pageState;
      const nextPoint = Vec43.sub(camera.point, Vec43.div(delta, camera.zoom));
      const nextZoom = zoom;
      const p0 = Vec43.sub(Vec43.div(point, camera.zoom), nextPoint);
      const p1 = Vec43.sub(Vec43.div(point, nextZoom), nextPoint);
      return this.setCamera(Vec43.toFixed(Vec43.add(nextPoint, Vec43.sub(p1, p0))), nextZoom, `pinch_zoomed`);
    });
    __publicField(this, "zoomTo", (next, center = this.centerPoint) => {
      const { zoom, point } = this.pageState.camera;
      const p0 = Vec43.sub(Vec43.div(center, zoom), point);
      const p1 = Vec43.sub(Vec43.div(center, next), point);
      return this.setCamera(Vec43.toFixed(Vec43.add(point, Vec43.sub(p1, p0))), next, `zoomed_camera`);
    });
    ...
    __publicField(this, "zoomToFit", () => {
      ...
      const commonBounds = Utils53.getCommonBounds(shapes.map(TLDR.getBounds));
      ...
      return this.setCamera(Vec43.toFixed(Vec43.sub([mx, my], [commonBounds.minX, commonBounds.minY])), zoom, `zoomed_to_fit`);
    });
    __publicField(this, "zoomToSelection", () => {
      ...
      return this.setCamera(Vec43.toFixed(Vec43.sub([mx, my], [selectedBounds.minX, selectedBounds.minY])), zoom, `zoomed_to_selection`);
    });
    __publicField(this, "zoomToContent", () => {
      ...
      const mx = (rendererBounds.width - commonBounds.width * zoom) / 2 / zoom;
      const my = (rendererBounds.height - commonBounds.height * zoom) / 2 / zoom;
      return this.setCamera(Vec43.toFixed(Vec43.sub([mx, my], [commonBounds.minX, commonBounds.minY])), this.pageState.camera.zoom, `zoomed_to_content`);
    });
    __publicField(this, "resetZoom", () => {
      return this.zoomTo(1);
    });
    __publicField(this, "zoomBy", Utils53.throttle((delta, center) => {
    ...
    __publicField(this, "nudge", (delta, isMajor = false, ids = this.selectedIds) => {
      ...
      return this.setState(translateShapes(this, ids, Vec43.mul(delta, size)));
    });
    ...
    __publicField(this, "group", (ids = this.selectedIds, groupId = Utils53.uniqueId(), pageId = this.currentPageId) => {
      if (this.readOnly)
        return this;
    ...
    __publicField(this, "onPan", (info, e) => {
      if (this.appState.status === "pinching")
        return;
      const delta = Vec43.div(info.delta, this.pageState.camera.zoom);
      const prev = this.pageState.camera.point;
      const next = Vec43.sub(prev, delta);
      if (Vec43.isEqual(next, prev))
        return;
      this.pan(delta);
    ...
    __publicField(this, "onPointerMove", (info, e) => {
      ...
      if (this.isForcePanning && this.isPointing) {
        (_a = this.onPan) == null ? void 0 : _a.call(this, __spreadProps(__spreadValues({}, info), { delta: Vec43.neg(info.delta) }), e);
        return;
      }
    ...
    __publicField(this, "onDoubleClickBoundsHandle", (info, e) => {
      ...
      if (shape.type === "image" /* Image */ || shape.type === "video" /* Video */) {
        ...
        const delta = Vec43.sub(centerB, centerA);
        this.updateShapes({
          id: shape.id,
          point: Vec43.sub(shape.point, delta),
          size: asset.size
        });
      }
    });
  ...
  createTextShapeAtPoint(point, id) {
    ...
    newShape.point = Vec43.sub(newShape.point, [bounds.width / 2, bounds.height / 2]);
  ...
  exportAllShapesAs(type) {
    return __async(this, null, function* () {
      ...
      this.selectAll();
      const { width, height } = Utils53.expandBounds(TLDR.getSelectedBounds(this.state), 64);
    ...
    return __async(this, null, function* () {
      ...
      try {
        ...
        const exportInfo = {
          ...
          type,
          serialized,
          size: type === "png" ? Vec43.mul(size, 2) : size
        };
  ...
  get centerPoint() {
    const { width, height } = this.rendererBounds;
    return Vec43.toFixed([width / 2, height / 2]);
  }
  get currentGrid() {
    const { zoom } = this.pageState.camera;
    if (zoom < 0.15) {
...
function useKeyboardShortcuts(ref) {
  ...
  useHotkeys("\u2318+shift+c,ctrl+shift+c", (e) => {
    if (!canHandleEvent())
      return;
    app.copySvg();
    e.preventDefault();
...
import { Utils as Utils54 } from "@tldraw/core";
var commandKey = () => Utils54.isDarwin() ? "\u2318" : "Ctrl";
function Kbd({
  variant,
  children
}) {
...
var RowButton = React33.forwardRef((_a, ref) => {
  ...
  return /* @__PURE__ */ React33.createElement(StyledRowButton, __spreadValues({
    ...
    variant: "menu"
  }, kbd) : void 0, hasIndicator && /* @__PURE__ */ React33.createElement(ItemIndicator, {
    dir: "ltr"
  }, /* @__PURE__ */ React33.createElement(SmallIcon, null, /* @__PURE__ */ React33.createElement(CheckIcon, null))), hasArrow && /* @__PURE__ */ React33.createElement(SmallIcon, null, /* @__PURE__ */ React33.createElement(ChevronRightIcon, null))));
});
...
var shapeShapeIcons = {
  ["rectangle" /* Rectangle */]: /* @__PURE__ */ React54.createElement(SquareIcon, null),
  ["ellipse" /* Ellipse */]: /* @__PURE__ */ React54.createElement(CircleIcon2, null),
  ["triangle" /* Triangle */]: /* @__PURE__ */ React54.createElement(VercelLogoIcon, null),
...
function DMItem(_a) {
  ...
  return /* @__PURE__ */ React56.createElement(Item2, {
    dir: "ltr",
    asChild: true,
    onSelect,
    id
...
function DMCheckboxItem({
  ...
  return /* @__PURE__ */ React57.createElement(CheckboxItem, {
    dir: "ltr",
...
function DMSubMenu({
  ...
  return /* @__PURE__ */ React59.createElement("span", {
    id
  }, /* @__PURE__ */ React59.createElement(Root3, {
    dir: "ltr"
    ...
    disabled,
    hasArrow: true
  }, label)), /* @__PURE__ */ React59.createElement(Content4, {
    dir: "ltr",
    asChild: true,
    sideOffset: 2,
    alignOffset: -2
...
function DMTriggerIcon(_a) {
  ...
  return /* @__PURE__ */ React60.createElement(Trigger3, {
    asChild: true,
    id
...
import { MixerVerticalIcon, Pencil1Icon as Pencil1Icon2 } from "@radix-ui/react-icons";
...
function PageOptionsDialog({ page, onOpen, onClose }) {
  ...
  return /* @__PURE__ */ React68.createElement(Dialog.Root, {
    ...
    bp: breakpoints
  }, /* @__PURE__ */ React68.createElement(SmallIcon, null, /* @__PURE__ */ React68.createElement(MixerVerticalIcon, null)))), /* @__PURE__ */ React68.createElement(Dialog.Portal, null, /* @__PURE__ */ React68.createElement(StyledDialogOverlay, {
    onPointerDown: close
    ...
    icon: /* @__PURE__ */ React68.createElement(Pencil1Icon2, null)
...
var ALIGN_ICONS = {
  ["start" /* Start */]: /* @__PURE__ */ React71.createElement(TextAlignLeftIcon, null),
  ["middle" /* Middle */]: /* @__PURE__ */ React71.createElement(TextAlignCenterIcon, null),
  ["end" /* End */]: /* @__PURE__ */ React71.createElement(TextAlignRightIcon, null),
  ["justify" /* Justify */]: /* @__PURE__ */ React71.createElement(TextAlignJustifyIcon, null)
};
...
import { CheckIcon as CheckIcon3, ClipboardIcon, CursorArrowIcon as CursorArrowIcon2 } from "@radix-ui/react-icons";
...
var MultiplayerMenu = React72.memo(function MultiplayerMenu2() {
  ...
  return /* @__PURE__ */ React72.createElement(DropdownMenu7.Root, {
  ...
  }, room ? /* @__PURE__ */ React72.createElement(MultiplayerIcon, null) : /* @__PURE__ */ React72.createElement(CursorArrowIcon2, null)), /* @__PURE__ */ React72.createElement(DMContent, {
    variant: "menu",
    align: "start",
    ...
    disabled: !room
  }, "Copy Invite Link", /* @__PURE__ */ React72.createElement(SmallIcon, null, copied ? /* @__PURE__ */ React72.createElement(CheckIcon3, null) : /* @__PURE__ */ React72.createElement(ClipboardIcon, null))), /* @__PURE__ */ React72.createElement(DMDivider, {
...
function AlignDistributeSubMenu({
  ...
  }, /* @__PURE__ */ React74.createElement(AlignLeftIcon2, null)), /* @__PURE__ */ React74.createElement(CMIconButton, {
    onClick: alignCenterHorizontal,
    id: "TD-ContextMenu-Align_Duplicate-AlignCenterHorizontal"
  }, /* @__PURE__ */ React74.createElement(AlignCenterHorizontallyIcon2, null)), /* @__PURE__ */ React74.createElement(CMIconButton, {
    onClick: alignRight,
    id: "TD-ContextMenu-Align_Duplicate-AlignRight"
  }, /* @__PURE__ */ React74.createElement(AlignRightIcon2, null)), /* @__PURE__ */ React74.createElement(CMIconButton, {
    onClick: stretchHorizontally,
    id: "TD-ContextMenu-Align_Duplicate-StretchHorizontal"
  }, /* @__PURE__ */ React74.createElement(StretchHorizontallyIcon2, null)), hasThreeOrMore && /* @__PURE__ */ React74.createElement(CMIconButton, {
    onClick: distributeHorizontally,
    id: "TD-ContextMenu-Align_Duplicate-SpaceEvenlyHorizontal"
  }, /* @__PURE__ */ React74.createElement(SpaceEvenlyHorizontallyIcon2, null)), /* @__PURE__ */ React74.createElement(CMIconButton, {
    onClick: alignTop,
    id: "TD-ContextMenu-Align_Duplicate-AlignTop"
  }, /* @__PURE__ */ React74.createElement(AlignTopIcon2, null)), /* @__PURE__ */ React74.createElement(CMIconButton, {
    onClick: alignCenterVertical,
    id: "TD-ContextMenu-Align_Duplicate-AlignCenterVertical"
  }, /* @__PURE__ */ React74.createElement(AlignCenterVerticallyIcon2, null)), /* @__PURE__ */ React74.createElement(CMIconButton, {
    onClick: alignBottom,
    id: "TD-ContextMenu-Align_Duplicate-AlignBottom"
  }, /* @__PURE__ */ React74.createElement(AlignBottomIcon2, null)), /* @__PURE__ */ React74.createElement(CMIconButton, {
    onClick: stretchVertically,
    id: "TD-ContextMenu-Align_Duplicate-StretchVertical"
  }, /* @__PURE__ */ React74.createElement(StretchVerticallyIcon2, null)), hasThreeOrMore && /* @__PURE__ */ React74.createElement(CMIconButton, {
    onClick: distributeVertically,
    id: "TD-ContextMenu-Align_Duplicate-SpaceEvenlyVertical"
  }, /* @__PURE__ */ React74.createElement(SpaceEvenlyVerticallyIcon2, null)), /* @__PURE__ */ React74.createElement(CMArrow, {
    offset: 13
  })))));
}
...
var InnerTldraw = React77.memo(function InnerTldraw2({
  ...
  return /* @__PURE__ */ React77.createElement(StyledLayout, {
    ...
    autofocus
  }), /* @__PURE__ */ React77.createElement(ContextMenu, null, /* @__PURE__ */ React77.createElement(Renderer, {
    id,
